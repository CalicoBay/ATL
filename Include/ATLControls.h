...
namespace ATLControls
{
	...
	template <class Base> class CBOMEditT;
	...
	/////////////////////////////////////////////////////////////////////////////
	// CBOMEdit - client side for a Windows EDIT control

	template <class Base>
	class CBOMEditT : public Base
	{
	public:
		CBomFile m_File;
		CStringW m_sActivePath;
		BOOL IsDirty(){ return GetModify(); }
	protected:
					...
					if(SUCCEEDED(hr))
					{
						...
						if(SUCCEEDED(hr))
						{
							...
							if(0 == iUTF)
							{
								...
								SetSel((int)cbLength, (int)cbLength, FALSE);
								//SetFocus();
							...
							else if(8 == iUTF)
							{
								...
								SetSel((int)(cbLength - 3), (int)(cbLength - 3), FALSE);
								//SetFocus();
	...
	public:
		// Constructors
		CBOMEditT<Base>(HWND hWnd = NULL) : Base(hWnd), m_cbShadowBuffer(0), m_UTF(0) { }
		~CBOMEditT<Base>(){ m_ShadowBuffer.Free(); }
		CBOMEditT< Base >& operator=(HWND hWnd)
		{
			m_hWnd = hWnd;
			return *this;
		}
			...
			if(__nullptr != pFindReplace)
			{
				if(_T('\0') != pFindReplace->lpstrFindWhat[0])
				{
					...
					if(SUCCEEDED(hr))
					{
						...
						//else
						//{
						::SetWindowText(m_hWnd, EM_GETSEL, WPARAM(&dwStartChar), LPARAM(&dwEndChar));
						//}
						...
						//else
						//{
						//::SetWindowText(m_hWnd, EM_SETSEL, WPARAM(iPosition), LPARAM(iPosition + sToFind.GetLength()));
						SetSel(iPosition, iPosition + sToFind.GetLength());
						//}
					}
				}
			}
	...
	typedef CBOMEditT<CWindow>     CBOMEdit;
